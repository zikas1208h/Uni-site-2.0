...
var generateMasterSchedule=function(courses,config){
  ...
  // Doctors: max 3 days/week (3 groups × 1 lecture each; multiple groups allowed on same day in different amphitheatres)
  // Assistants: max 3 days/week, max 5 sections/day
  var DOC_MAX_DAYS=3,DOC_MAX_PER_DAY=4,AST_MAX_DAYS=3,AST_MAX_SLOT=5;

  // PASS 1: LECTURES — 1 lecture per group per week, doctor covers 3 groups each
  for(var ci=0;ci<courses.length;ci++){
    ...
    for(var group=1;group<=NUM_GROUPS;group++){
      // 2 doctors: A → groups 1-3, B → groups 4-6
      ...
      outer1:for(var di=0;di<days.length;di++){
        var day=days[di];
        if(!tracker.canUse(docId,day,DOC_MAX_DAYS,DOC_MAX_PER_DAY))continue;
  ...
  // PASS 2: SECTIONS — 1 section per group×section per week (G1S1=1 slot, G1S2=1 slot, ... 12 total)
  for(var ci2=0;ci2<courses.length;ci2++){
    ...
    // 12 jobs: each group has section 1 AND section 2, each gets its own distinct slot
    var allJobs=[];
